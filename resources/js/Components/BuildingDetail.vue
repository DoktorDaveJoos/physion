...
if (props.order) {
  ...
  form.construction_year = product.construction_year;
  form.construction_year_heating = product.construction_year_heating;
  form.floor_area = product.floor_area;
  form.housing_units = parseInt(product.housing_units);
  form.ventilation = product.ventilation;
  form.cellar = product.cellar;
  form.renewables = product.renewables;
  form.renewables_reason = product.renewables_reason;
  form.cooling = product.cooling;
  form.cooling_count = product.cooling_count ? parseInt(product.cooling_count) : null;
  form.cooling_service = product.cooling_service;

  form.suggestion_check = JSON.parse(product.suggestion_check ?? '{}');
}

const submit = () => {
  form.put(route(`${props.context}.details.update`, props.order.id));
};
...
<template>
  <el-form
    :model='form'
    class='pb-12 grid sm:grid-cols-2 sm:gap-x-8'
    label-position='top'
    size='large'
    ...
    <form-header :switcher='hasRenewables'
    ...
    <form-header :switcher='hasCooling'
    ...
    <form-header subtitle='Machen Sie Angaben zu bereits erfolgten Energiesparmaßnahmen'
                 title='Energetische Zusatzinformationen' />

    <div class='flex flex-col'>
      <el-checkbox
        v-model='form.suggestion_check.attic'
        label='Dach-/Dachbodendämmung'
      ></el-checkbox>
      <el-checkbox
      ...
      <el-checkbox
        ...
        label='Wärmeschutz-/Isolierverglasung'
      ></el-checkbox>
      <el-checkbox
      ...
      <el-checkbox
        ...
        label='Ausschließlich LED Leuchtmittel'
      ></el-checkbox>
    </div>
