...
//     roofForms.find((roofForm) => roofForm.title === title).title;

const roof = computed(() => props.roof);
    ...
    async (value, oldValue) => {
        ...
        if (
            ...
            ElMessageBox.confirm(
                `Für diese Dachform sind keine Gauben vorgesehen. Bestehende (${roof.dormers?.length}) Gauben werden gelöscht. Fortfahren?`,
                'Achtung',
                {
        ...
        if (value.title === props.roof?.roof_shape) {
            form.reset(
                'heated',
                'construction',
                'u_value',
...
const hasAdditional = computed(() => {
    return (
        props.roof?.insulations?.length > 0 ||
        props.roof?.windows?.length > 0 ||
        props.roof?.dormers?.length > 0
    );
});
</script>
...
<template>
    ...
    <template v-if="form.roof_shape">
        <!--   Homemade divider   -->
        <div class="border-t border-gray-100 flex mb-5"></div>
        ...
        <el-form
            ...
            <el-form-item
                ...
                <el-cascader
                    ...
                    :options="options[form.roof_shape.toLowerCase()]"
                    :props="cascaderProps"
                    class="w-full"
        ...
        <flachdach-form
            ...
            :form="form" />
        <flachdach-form
            v-else-if="form.roof_shape === 'Flachdach' && form.construction[0]"
            :form="form" />
        <pultdach-form
            v-else-if="form.roof_shape === 'Pultdach' && form.construction[0]"
            :form="form" />

        <template v-if="hasAdditional">
            <div
                class="border-t border-gray-200 shadow-inner px-6 py-4 space-y-1 bg-gray-50">
                <div v-if="roof?.insulations?.length > 0">
                ...
                <div
                    v-for="insulation in roof?.insulations"
                ...
                <div v-if="roof?.windows?.length > 0">
                    <span class="text-xs text-gray-500">Dachfenster</span>
                </div>

                <div
                    v-for="skylight in roof?.windows"
                    class="flex rounded-lg bg-white border border-gray-200 p-2 items-center">
                    <div
                        class="h-12 w-12 mr-4 bg-gray-100 rounded-lg flex justify-center items-center">
                ...
                <div v-if="roof?.dormers?.length > 0">
                    <span class="text-xs text-gray-500">Gauben</span>
                </div>

                <div
                    v-for="dormer in roof?.dormers"
                    class="flex rounded-lg bg-white border border-gray-200 p-2 items-center">
                    <div
                        class="h-12 w-12 mr-4 bg-gray-100 rounded-lg flex justify-center items-center">
        ...
        <div
            ...
            <bz-button
                v-if="form.roof_shape === 'Satteldach'"
                type="secondary"
        ...
        <el-drawer v-model="state.skylight">
            ...
            <el-form label-position="top" size="large">
                <el-form-item label="Anzahl" :error="skylightForm.errors.count">
                    <el-input-number
    ...
    <div
        ...
        class="w-full flex justify-end p-5 border-t border-gray-100">
