...
export default {
    ...
    setup(props) {
        ...
        const hasVacancy = ref(
            props.product.vacancies.length > 0 ||
                props.product.vacancy_percentage !== null
        );
        const activeName = ref(
            props.product.vacancies.length > 0 ? 'period' : 'percentage'
        );
        ...
        const addSource = () => {
            Inertia.put(
                route('verbrauch.consumption.source.update', props.order.id),
                addSourceForm,
                {
                    onProgress: () => {
                        drawer.loading = true;
        ...
        const addPeriod = (sourceId) => {
            Inertia.post(
                route('verbrauch.consumption.period.create', {
        ...
        const deletePeriod = (sourceId, periodId) => {
            Inertia.delete(
                route('verbrauch.consumption.period.delete', {
                    order: props.order.id,
                    source: sourceId,
                    period: periodId,
                }),
        ...
        const deleteSource = (sourceId) => {
            ElMessageBox.confirm('Energieträger wirklich löschen?', 'Löschen', {
                ...
                .then(() => {
                    Inertia.delete(
                        route('verbrauch.consumption.source.delete', {
                            order: props.order.id,
                            source: sourceId,
                        }),
                        {
        ...
        const addVacancy = () => {
            ...
            addVacancyForm.period[1] = dayjs(addVacancyForm.period[1])
                .endOf('month')
                .toISOString();

            Inertia.post(
                route('verbrauch.consumption.vacancy.create', props.order.id),
                addVacancyForm,
                {
                    preserveScroll: true,
                    onSuccess: () => {
        ...
        const deleteVacancy = (vacancyId) => {
            Inertia.delete(
                route('verbrauch.consumption.vacancy.delete', {
                    order: props.order.id,
                    vacancy: vacancyId,
                }),
                {
        ...
        const getMonths = (source) => {
            ...
            source.periods.forEach((period) => {
                months +=
                    dayjs(period.start).diff(dayjs(period.end), 'month') * -1 +
                    1;
            });
        ...
        const markDone = () => {
            Inertia.put(
                route('verbrauch.consumption.done', props.order.id),
                markDoneForm,
                {
                    preserveScroll: true,
                }
        ...
        <template #header>
            <div class="flex flex-col">
                ...
                <p class="text-xs text-gray-500">
                    Falls Sie über mehrere Energiequellen verfügen, können Sie
            ...
            <div class="flex flex-col space-y-4">
                <el-form label-position="top">
                    ...
                    <el-form-item label="Warmwasser" :error="errors.water">
                        <el-select
                            v-model="addSourceForm.water"
                            size="large"
                            class="w-full"
                            placeholder="Bitte auswählen">
                            <el-option
                                default-first-option
                                label="Im Verbrauch enthalten - Genaue Werte unbekannt"
                                value="Im Verbrauch enthalten - Genaue Werte unbekannt" />
                            <el-option
                                default-first-option
                                label="Im Verbrauch enthalten - Genaue Werte bekannt"
                            ...
                            <el-option
                                default-first-option
                                label="Nicht im Verbrauch enthalten - Genaue Werte bekannt"
                ...
                <el-tabs v-model="activeName" class="sm:col-span-2">
                    <el-tab-pane label="Prozentual" name="percentage">
                        <el-form-item
                            ...
                            <el-input
                                v-model="markDoneForm.percentage"
                                size="large">
                                <template #append>%</template>
                            </el-input>
                        </el-form-item>
                        <el-alert
                            v-if="product.vacancies.length > 0"
                            type="info"
                            show-icon>
                    ...
                    <el-tab-pane label="Genauer Zeitraum" name="period">
                        <table class="min-w-full divide-y divide-gray-300">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                <tr
                                    v-for="period in product.vacancies"
                                    :key="period.id">
                                    <td
                                        class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 md:pl-0">
                                        {{
                        ...
                        <el-alert
                            ...
                            show-icon />
                        <el-alert
                            v-if="product.vacancy_percentage"
