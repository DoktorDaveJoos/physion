                    ...
                    <div class="mt-6 grid gap-16 pt-10 lg:gap-x-5 lg:gap-y-12">
                        ...
                        <el-timeline class="hidden sm:block" center>
                            <el-timeline-item v-for="post in posts" :key="post.id" center>
                                <InertiaLink :href="route('blog.show.post', post.slug)">
                                    <div class="p-4 rounded hover:bg-slate-50 transition duration-150">
                        ...
                        <div class="grid sm:hidden divide-y">
                            <InertiaLink v-for="post in posts" :key="post.id" :href="route('blog.show.post', post.slug)"
                                         class="py-6">
                                <p class="text-sm text-gray-500">
                                    <time
                                        :datetime="dayjs(post.published_at).locale('de').format('D MMM YYYY')">
