...
<template>
    <div class="rounded-lg shadow bg-white mt-4 flex flex-col">
        ...
        <div class="border-t border-gray-100">
            <el-empty
                v-if="
                    building.data.heatings?.length === 0 &&
                    building.data.renewables?.length === 0
                "
                ...
                <ul role="list" class="divide-y divide-gray-100">
                    <li class="flex px-6 py-4 items-center justify-between">
                        <div class="flex items-center">
                            <check-circle-icon
                                v-if="building.data.heatings?.length > 0"
                        ...
                        <bz-button
                            ...
                            as="link"
                            :href="
                                route('buildings.energy.show', building.data.id)
                            "
                            type="primary"
                            plain
                    ...
                    <li class="flex px-6 py-4 items-center justify-between">
                        <div class="flex items-center">
                            <check-circle-icon
                                v-if="building.data.renewables?.length > 0"
                                class="h-5 w-5 text-emerald-500 mr-2 opacity-70" />
                            <x-circle-icon
                                v-else
