...
const addHeatingSystem = () => {
    form.transform((data) => ({
        ...
        is_main: props.building.data.heatingSystems?.length === 0,
    })).put(
        route('building.heating', {
            building: props.building.data.id,
        }),
        {
            preserveScroll: true,
...
const deleteRenewable = (id) => {
    form.delete(
        route('building.renewable.delete', {
            building: props.building.data.id,
