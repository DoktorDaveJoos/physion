...
const markDoneForm = useForm({
    vacancy_percentage: props.order.certificate?.vacancy_percentage ?? null,
});
...
const hasVacancy = ref(
    props.order.certificate?.vacancies?.length > 0 ||
        props.order.certificate?.vacancy_percentage !== null
);
const activeName = ref(
    props.order.certificate?.vacancies?.length > 0 ? 'period' : 'percentage'
);
        ...
        .post(
            ...
            }),
            {
                ...
                onSuccess: () => {
                    ...
                    markDoneForm.vacancy_percentage = null;
                },
                onError: () => {
                    addVacancyForm.reset();
                },
...
const mainSourceHas36Months = computed(() => {
    ...
    props.order.certificate?.sources.forEach((source) => {
        if (source.main) {
...
const shortcuts = [2022, 2021, 2020, 2019, 2018, 2017, 2016, 2015, 2014].map(
    (year) => {
        return {
            text: year.toString(),
            value: () => {
