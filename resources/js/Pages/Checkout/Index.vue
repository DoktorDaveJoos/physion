...
import {CalculatorIcon, FireIcon, ShieldCheckIcon} from '@heroicons/vue/24/outline';
...
export default {
    ...
    setup(props) {
        ...
        const addUpsell = (upsell) => {
            Inertia.post(route('checkout.upsell.add', {order: props.order.id, upsell: upsell}));
        };

        const removeUpsell = (upsell) => {
            Inertia.delete(route('checkout.upsell.delete', {order: props.order.id, upsell: upsell}));
        };

        // Total price
        const total = computed(() => {
            return props.addedUpsells.reduce((total, upsell) => {
        ...
        <div class="mx-auto max-w-2xl px-4 pt-16 pb-24 sm:px-6 lg:max-w-7xl lg:px-8">
            ...
            <form class="mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
                    ...
                    <ul role="list" class="divide-y divide-gray-200 border-t border-b border-gray-200">
                        <li v-for="(product, productIdx) in products" :key="product.id" class="flex py-6 sm:py-10">
                            <div class="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                                <div class="relative pr-9 flex items-center">
                                    <div
                                        ...
                                        <component :is="map[order.type].icon" class="h-8 w-8" aria-hidden="true" />
                                    </div>
                        ...
                        <li v-for="(addedUpsell) in addedUpsells" :key="addedUpsell.id" class="flex py-6 sm:py-10">
                            <div class="w-full grid sm:grid-cols-3">
                                <div class="ml-4 flex flex-1 flex-col justify-between sm:ml-6 sm:col-span-2">
                                    <div class="relative pr-9 flex items-center">
                                        <div
                                            class="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-md text-white sm:h-14 sm:w-14 bg-slate-200">
                                            <ShieldCheckIcon class="h-8 w-8 text-slate-800" aria-hidden="true" />
                                        </div>
                                        ...
                                        <div class="flex flex-col ml-4">
                                            ...
                                            <p class="text-sm font-medium text-gray-900">{{ addedUpsell.price }} €</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="flex justify-end">
                                    <el-button text type="default" @click="removeUpsell(addedUpsell.id)">
                                        <XMarkIcon class="h-5 w-5 text-gray-400 hover:text-gray-400 cursor-pointer"
                                                   aria-hidden="true" />
                                    </el-button>
                                </div>
                            </div>
                            ...
                            <div class="mx-auto max-w-7xl">
                                <div class="mx-auto max-w-2xl px-4 lg:max-w-4xl lg:px-0">
                                    <h1 class="text-2xl font-bold tracking-tight text-gray-900 sm:text-2xl">
                                        Empfehlungen</h1>
                                    <p class="mt-2 text-sm text-gray-500">Schützen Sie Ihre Investition in den
                                        Energieausweis und versichern
                                        Sie sich gegen eventuelle Fehleingaben.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <div class="mx-auto max-w-7xl ">
                                    <div class="mx-auto max-w-2xl space-y-8 sm:px-4 lg:max-w-4xl lg:px-0">
                                        <div v-for="upsell in upsells" :key="upsell.id"
                                                ...
                                                <li class="p-4 sm:p-6">
                                                    <div class="flex items-center sm:items-start">
                                                        <div
                                                            ...
                                                            <ShieldCheckIcon class="h-8 w-8 text-slate-800"
                                                                             aria-hidden="true" />
                                                        </div>
                                                        <div class="ml-6 flex-1 text-sm">
                                                            <div
                                                                ...
                                                                <div class="flex items-center">
                                                                    <h5>{{ upsell.name }}</h5>
                                                                    <span v-if="upsell.recommended"
                                                                          class="inline-flex ml-2 items-center rounded-full bg-green-100 px-3 py-0.5 text-sm font-medium text-green-800">Empfehlung</span>
                                                                </div>
                                                                <p class="mt-2 sm:mt-0">{{
                                                                        upsell.price.toString().
                                                                            replace('.', ',')
                                                                    }} €</p>
                                                            </div>
                ...
                <section aria-labelledby="summary-heading"
                    ...
                    <h2 id="summary-heading" class="text-lg font-medium text-gray-900">Bestellübersicht</h2>

                    <dl class="mt-6 space-y-4">
                        <div class="flex items-center justify-between">
                            <dt class="text-sm text-gray-600">Verbrauchsausweis</dt>
                        ...
                        <div class="flex items-center justify-between border-t border-gray-200 pt-4">
                            ...
                            <dd class="text-sm font-medium text-gray-900">{{
                                    addedUpsells.reduce((prev, cur) => prev + parseFloat(cur.price), 0).
                                        toFixed(2).
                                        toString().
                                        replace('.', ',')
                                }} €
                    ...
                    <div class="mt-6">
                        <PaypalButton :price="total" :order="order" />
                    </div>
                </section>
            </form>
        </div>
